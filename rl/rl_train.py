...
class KubernetesEnvironment:
    ...
    def get_state(self):
        try:
            ...
            df.loc[len(df.index)]=[str(utilization)]
    ...
    def step(self, action):
        try:
            ...
            if utilization>0.2 and utilization<0.6:
...
class KubernetesAgent:
    ...
    def act(self, state):
        if np.random.rand() <= self.epsilon:
            ...
            return np.argmax(self.q_network.predict(np.array([state]).reshape((1,1)))[0]) - 1
        
    def train(self, batch_size=2, gamma=0.99):
...
# Create a callback that saves the model's weights
        cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path,
                                                 ...
                                                 verbose=1)
        history=self.q_network.fit(np.array(batch_states).reshape((2,1)), np.array(batch_targets), epochs=1, verbose=0,callbacks=[cp_callback])
